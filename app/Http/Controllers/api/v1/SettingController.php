...
{
    // get setting to version in applications 
    public function getSetting(Request $request)
    {
        ...
        if (!empty($build_no && $type && $os)) {
            $setting = Setting::where('type', 'customer_app')
                ->orderBy('created_at')
                ->get();
            ...
            if ($versions) {
                ...
                return response()->json(['status' => 1, 'message' => __('site.messages.success'), 'versionInfo' => $version, 'settings' => $settings,'locationUser'=>$data]);
            } else {
                return response()->json(['status' => 1, 'message' => __('site.messages.success'), 'versionInfo' => $version, 'settings' => $settings,'locationUser'=>$data]);
            }
        }else{
